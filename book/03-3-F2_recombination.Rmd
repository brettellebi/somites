...
# F2 recombination blocks (no reads overlapping repeat regions and no sites with persistent heterozygosity)
...
```{r}
in_dir = "/nfs/research/birney/users/ian/somites/recombination_blocks/F2/no_repeat_reads_or_pers_hets"
...
```{r, eval = F}
saveRDS(base_cov_df_chr, here::here("data/20211123_base_cov_per_chr.rds"))
```

```{r, include = F}
base_cov_df_chr = readRDS(here::here("data/20211123_base_cov_per_chr.rds"))
```
...
```{r}
ggsave(here::here("plots/20211123_gt_freqs_sites.png"),
       device = "png",
       width = 10,
       height = 5.8,
       units = "in",
...
```{r, eval = F, echo = F}
saveRDS(bb_list, here::here("data/20211123_bb_list.rds"))
```

```{r, include = F}
bb_list = readRDS(here::here("data/20211123_bb_list.rds"))
```
...
purrr::map(bb_list, function(block_bounds_list){
  ...
  file_out = here::here("book/plots/20211123_karyoplot_no_repeats_no_pers_hets/no_gaps", file_name)
...
knitr::include_graphics(here::here("book/plots/20211123_karyoplot_no_repeats_no_pers_hets/no_gaps/5000.png"))
```
...
```{r, fig.cap = "Bin length: 5 kb"}
knitr::include_graphics(here::here("book/plots/20211123_karyoplot_no_repeats_no_pers_hets/wi_gaps/5000.png"))
```
