...
```{r}
final_wmiss = purrr::map(seq_along(gt_list), function(COUNTER){
  out = list()
...
names(final_wmiss) = names(gt_list)
```
...
```{r}
test_out_wmiss = file.path(out_dir_test, "gwas_results", "gwas_results_wmiss.rds")
```

```{r, eval = F}
gwas_tests_nomiss = purrr::map(final_wmiss, function(BIN_LENGTH){
  run_gwas(d = BIN_LENGTH$genotypes,
           m = BIN_LENGTH$positions,
           p = BIN_LENGTH$phenotypes)
})
