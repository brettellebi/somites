...
```{r}
in_file = "/nfs/research/birney/users/ian/somites/recombination_blocks/hmm_output_batch_01_corrected.txt"
...
```{r}
full_data = readr::read_tsv(in_file,
                            col_types = "ciiidii")
...
This shows that state `0` is associated with a *low* ratio of Cab to Kaga, and therefore a call of *Kaga*. Vice versa: state `2` and *Cab*. 
...
```{r}
full_data %>% 
  # Get total read counts per bin
  dplyr::mutate(total = mat + pat ) %>% 
  # Plot
  ggplot(aes()) +
    geom_violin() +
...
```{r, eval = F, echo = F}
saveRDS(block_bounds_list, here::here("data/20210716_block_bounds_list.rds"))
```

```{r, include = F}
block_bounds_list = readRDS(here::here("data/20210716_block_bounds_list.rds"))
```
...
```{r, eval = F}
file_out = here::here("docs/plots/20210716_karyoplot.png")
...
```{r}
knitr::include_graphics(here::here("docs/plots/20210716_karyoplot.png"))
```
