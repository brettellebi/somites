...
SAMPLE_GTS_DIR = file.path("/hps/nobackup/birney/users/ian/somites/association_testing",
                           date_of_assoc_test,
                           "all_sites/sample_genos")
RESULTS_DIR = file.path("/hps/nobackup/birney/users/ian/somites/association_testing",
                        date_of_assoc_test)
```
...
sample_genos_list = purrr::map(TARGET_FILES, function(FILE){
  ...
  n_cols = readr::count_fields(FILE, tokenizer_csv())[1]
  # set column types
  COL_TYPES = paste("c", paste(rep("i", n_cols -1 ), collapse = ""), sep = "", collapse = "")
  # read in files
