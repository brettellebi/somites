...
plot_dat = purrr::map(target_phenos, function(PHENO){
  bin_list = purrr::map(bin_lengths, function(BIN_LENGTH){
...
# Plot
all_sites_plots = purrr::map(target_phenos, function(PHENO){
  bins_out = purrr::map(bin_lengths, function(BIN_LENGTH){
    
    BIN_L = BIN_LENGTH
    
    # Get significant level
    SIG_LEVEL = perms_df_mins %>%
      dplyr::filter(SITE_FILTER == "all_sites",
                    TARGET_PHENO == PHENO,
                    BIN_LENGTH == BIN_L) %>% 
      dplyr::pull(MIN_P) %>% 
      -log10(.)
    ...
    # Plot
    out = plot_man(plot_dat[[PHENO]][[as.character(BIN_LENGTH)]],
    ...
    ggsave(file.path(plot_dir, paste(PHENO, "_", BIN_L, ".png", sep = "")),
           out,
           device = "png",
           width = 9.6,
           height = 6,
  ...
  names(bins_out) = bin_lengths
})

names(all_sites_plots) = target_phenos
```
